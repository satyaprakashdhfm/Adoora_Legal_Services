import { firm } from "@/content/firm";

/**
 * Floating WhatsApp button, fixed to the bottom-right corner of every page
 * (see the root layout, which keeps it off `/notice`).
 *
 * It opens a chat with the firm's contact number. There is deliberately no
 * separate WhatsApp number to keep in step: `wa.me` wants the E.164 number
 * with the `+` dropped, which `firm.phoneE164` already is.
 *
 * Two thin rings expand from the button's edge and fade to nothing,
 * staggered so there is always one mid-flight (`.whatsapp-ripple` in
 * globals.css, which also turns them off for reduced motion). Each is an
 * outline only — unfilled, so the page shows through inside and outside the
 * ring — and an SVG stroke rather than a CSS border: `non-scaling-stroke`
 * keeps the line the same hairline width however far the ring has grown,
 * where a scaled border would thicken as it went. They take no pointer
 * events, so they never enlarge the link's click target.
 *
 * Sits at `z-40`, deliberately under the cookie banner (`z-50`, opaque, and
 * with its Accept / Reject buttons at the right-hand end): while the banner
 * is up it covers this rather than this covering its buttons, and the button
 * appears once a choice is made. Chasing the banner's height instead would
 * mean tracking its state and its responsive, expandable size.
 *
 * WhatsApp's own green rather than a site token — it is the recognisable
 * part of the mark, and it should read as WhatsApp against any band it
 * happens to float over.
 */
export function WhatsAppButton() {
  const href = `https://wa.me/${firm.phoneE164.replace(/^\+/, "")}`;

  return (
    <a
      href={href}
      target="_blank"
      rel="noopener noreferrer"
      aria-label="Chat with us on WhatsApp"
      className="fixed bottom-5 right-5 z-40 flex h-12 w-12 items-center justify-center rounded-full bg-[#25D366] text-white shadow-[0_8px_22px_rgba(11,24,52,0.3)] transition duration-200 hover:scale-105 hover:shadow-[0_10px_26px_rgba(11,24,52,0.38)] sm:bottom-6 sm:right-6 print:hidden"
    >
      <svg
        viewBox="0 0 100 100"
        aria-hidden="true"
        className="whatsapp-ripple pointer-events-none absolute inset-0 h-full w-full overflow-visible"
      >
        <circle
          cx="50"
          cy="50"
          r="50"
          fill="none"
          stroke="#25D366"
          strokeWidth="1.5"
          vectorEffect="non-scaling-stroke"
        />
      </svg>
      <svg
        viewBox="0 0 100 100"
        aria-hidden="true"
        className="whatsapp-ripple pointer-events-none absolute inset-0 h-full w-full overflow-visible [animation-delay:1.2s]"
      >
        <circle
          cx="50"
          cy="50"
          r="50"
          fill="none"
          stroke="#25D366"
          strokeWidth="1.5"
          vectorEffect="non-scaling-stroke"
        />
      </svg>
      <svg
        viewBox="0 0 24 24"
        aria-hidden="true"
        className="relative h-7 w-7"
        fill="currentColor"
      >
        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
      </svg>
    </a>
  );
}
